...
public class CartController {
  ...
  public @ResponseBody ResponseEntity orderCartItem
    ...
    if(cartOrderDtoList == null || cartOrderDtoList.size() == 0) { //주문할 상품 선택했는지 체크
    ...
    Long orderId = cartService.orderCartItem(cartOrderDtoList, principal.getName());  //주문 로직 호출 결과 생성된 주문번호 반환
    return new ResponseEntity<Long>(orderId, HttpStatus.OK);  //주문번호가 생공적으로 생성되면 주문번호와 성공http응답 코드 반환
  }
}
